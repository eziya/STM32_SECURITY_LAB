                                          ...
                                          SE_FwRawHeaderTypeDef *pTestHeader)
{
  ...
  /* Write Magic */
  //valid 헤더와 test 헤더를 반씩 merge 하여 swap magic 영역에 기록한다.
  if (e_ret_status == SFU_SUCCESS)
  {
...
static SFU_ErrorStatus CheckTrailerMagic(uint32_t DwlSlot)
{
   ...
   */
  //swap magic 값이 valid 헤더와 test 헤더의 merge 값인지 확인한다.
                                          ...
                                          SE_FwRawHeaderTypeDef *pTestHeader)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    //설치하려고 트레일러에 기록한 헤더와 실제 다운로드 슬롯에 있는 fw 헤더가 동일한지 확인한다.
  ...
  //fw 를 설치하려고 하는 액티브 슬롯의 헤더를 읽어온다.
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      //액티브 헤더가 valid 헤더와는 일치하지 않지만 테스트 헤더와 일치하는 경우에도 설치한다.
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      if (e_ret_status == SFU_SUCCESS)
      {
        ...
        if (e_ret_status == SFU_SUCCESS)
        {
          ...
          //valid 헤더와 test 헤더의 버전을 비교하여 설치 가능한지 확인한다.
          e_ret_status = SFU_IMG_CheckFwVersion(ActiveSlot, pValidHeader->FwVersion, pTestHeader->FwVersion);
        }
      }
      else
      {
        ...
        //valid 헤더에 정보가 없는 경우는 초기 설치인 경우일 수 있다.
                                                           ...
                                                           SE_FwRawHeaderTypeDef *pFwImageHeader)
{
  ...
  if (se_ret_status == SE_SUCCESS)
  {
     ...
     * Skip header : no decryption needed
     */
    uint32_t fw_index = SFU_IMG_IMAGE_OFFSET;
...
/**
  ...
  */
//swap 영역을 사용하지 않기 때문에 액티브 영역의 이미지를 복구할 수는 없지만 코드는 훨씬 간단하다.
                                         ...
                                         SE_FwRawHeaderTypeDef *pTestHeader)
{
   ...
   */
  //다운로드 슬롯의 헤더를 decrypt 하기 전 검증한다.
   ...
   */
  //인스톨을 시작하는 것을 기록하기 위해 트레일러 정보를 기록한다.
   ...
   */
  //swap 을 사용하지 않기 때문에 다운로드 영역에서 바로 액티브 슬롯 영역으로 decrypt 한다.
   ...
   */
  //다운로드 영역의 복호화가 정상적으로 완료되면 다운로드 슬롯을 erase 한다.
...
static SFU_ErrorStatus FirmwareToInstall(uint32_t DwlSlot, SE_FwRawHeaderTypeDef *pFwImageHeader)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    //트레일러 영역을 fw 이미지가 침범하지 않는지 확인한다.
                                                                ...
                                                                uint32_t *pActiveSlotToRollback)
{
  ...
  //swap 영역을 사용하지 않기 때문에 rollback 은 지원하지 않는다.
  ...
  *pDwlSlotToInstall = SLOT_INACTIVE;
  ...
  for (i = 0U; i < SFU_NB_MAX_DWL_AREA; i++)
  {
    ...
    {
      //다운로드 슬롯에 정상적인 fw 헤더 정보가 있는 경우 설치를 시작할 수 있다.
...
/**
  ...
  */
//설치가 중단된 경우, 정상적으로 설치가 중단되어야 다운로드 슬롯이 삭제되기 때문에 중단된 경우에는
//아직 다운로드 슬롯에 암호화된 fw 가 남아있다.
SFU_ErrorStatus SFU_IMG_TriggerResumeInstallation(uint32_t ActiveSlot, uint32_t DwlSlot)
{
   ...
   * Decrypt the FW image : from dwl slot to active slot
   */
  //다운로드 슬롯에 있는 암호화된 이미지를 액티브 슬롯에 복호화해준다.
   ...
   * erase downloaded FW after installation
   */
  //정상적으로 복호화가 완료되었다면 다운로드 슬롯을 erase 한다.
  e_ret_status = SFU_LL_FLASH_Erase_Size(&x_flash_info, (uint8_t *) SlotStartAdd[DwlSlot], SLOT_SIZE(DwlSlot));
  if (e_ret_status != SFU_SUCCESS)
  {
    STATUS_FWIMG(e_ret_status == SFU_ERROR, SFU_IMG_FLASH_ERASE_FAILED);
...
SFU_ErrorStatus SFU_IMG_TriggerImageInstallation(uint32_t DwlSlot)
{
   ...
   */
  //설치할 액티브 슬롯, 다운로드 슬롯 정보와 함께 양쪽 슬롯의 헤더 정보를 이용하여 설치를 진행한다.
  ...
  if (SFU_SUCCESS != e_ret_status)
  {
    ...
    //설치하다가 오류가 발생한 경우에는 다운로드 슬롯을 지워준다.
